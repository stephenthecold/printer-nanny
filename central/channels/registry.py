...
def build_channel(row: m.NotificationChannel) -> NotificationChannel:
    impl = _IMPLS[row.type]
    return impl(name=row.name, config=row.config or {})


def dispatch(
    note: Notification, channel_rows: Iterable[m.NotificationChannel]
