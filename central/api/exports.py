...
def _csv_response(filename: str, header: list[str], rows: Iterable[list]) -> StreamingResponse:
    ...
    spreadsheet-formula escaping. Both layers are needed and they solve
    different problems: csv.writer makes the file *parse* correctly (commas,
    quotes, newlines, RFC 4180), and csv_safe stops a cell from being
    *evaluated* when the operator opens it in Excel. Model names, locations and
    prtAlertDescription text in these exports come straight off printers on
    customer LANs, so a cell beginning "=" is a live formula the moment the
    file is double-clicked. See central/csv_safe.py.
    """
