...
.DESCRIPTION
    ...
    Or with a single-use claim code, letting the agent enroll itself:

        .\install-agent.ps1 -CentralUrl https://CENTRAL -ClaimCode pnc_xxxxx
...
param(
    ...
    [string]$ClaimCode = $env:PN_CLAIM_CODE,
...
# Required args. Two enrollment shapes: pre-minted credentials, or a single-use
# claim code the agent redeems itself on first start. Exactly one -- accepting
# both would leave it ambiguous which identity this box ends up with.
$useClaim = -not [string]::IsNullOrWhiteSpace($ClaimCode)
if ($useClaim -and (($null -ne $AgentId) -or -not [string]::IsNullOrWhiteSpace($ApiKey))) {
    throw "pass either -ClaimCode or -AgentId/-ApiKey, not both"
}
$required = if ($useClaim) { @('CentralUrl') } else { @('CentralUrl','AgentId','ApiKey') }
foreach ($name in $required) {
    $v = Get-Variable $name -ValueOnly -ErrorAction SilentlyContinue
    if ($null -eq $v -or $v -eq '') {
        throw "missing -$name (or set environment variable PN_$($name.ToUpper().Replace('CENTRALURL','CENTRAL_URL').Replace('AGENTID','AGENT_ID').Replace('APIKEY','API_KEY'))). Use -ClaimCode to self-enroll instead."
    }
}
...
if ($useClaim) {
    Write-Host "    agent   : self-enrolling with claim code on first start"
} else {
...
# With a claim code the agent redeems it on first start and rewrites this file
# with the credentials it was issued -- the (single-use) code is gone from disk
# after that, and nothing long-lived was ever transmitted.
$cfg = if ($useClaim) { @"
