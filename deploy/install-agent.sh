...
#
# Or with a single-use claim code instead, and let the agent enroll itself:
#
#   curl -fsSL https://CENTRAL/install-agent.sh | sudo bash -s -- \
#     --central-url https://CENTRAL --claim-code pnc_xxxxx
#
...
# Two enrollment shapes: pre-minted credentials, or a single-use claim code the
# agent redeems itself on first start. Exactly one is required -- accepting both
# would leave it ambiguous which identity the box ends up with.
if [ -n "$CLAIM_CODE" ]; then
  [ -z "$AGENT_ID$API_KEY" ] || die "pass either --claim-code or --agent-id/--api-key, not both"
else
  [ -n "$AGENT_ID" ] || die "--agent-id is required (or use --claim-code)"
  [ -n "$API_KEY" ] || die "--api-key is required (or use --claim-code)"
fi
case "$PIP_SOURCE" in
...
if [ -n "$CLAIM_CODE" ]; then
  echo "    agent   : self-enrolling with claim code on first start"
else
  echo "    agent   : #$AGENT_ID"
fi
...
mkdir -p "$CONFIG_DIR"
if [ -n "$CLAIM_CODE" ]; then
  # The agent redeems this on first start and rewrites this file with the
  # credentials it was issued. The code is single use, so it is gone from the
  # file after that -- nothing long-lived was ever transmitted or left on disk.
